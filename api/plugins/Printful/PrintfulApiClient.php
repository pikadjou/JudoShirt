...
namespace App\Model\Printful;


require_once(ROOT . DS . 'plugins' . DS  . 'Printful' . DS . 'Exceptions' . DS . 'PrintfulException.php');

use App\Model\Printful\Exceptions;
 ...
 class PrintfulApiClient
 {
    ...
    public static function getInstance()
    {
        if(Self::$_instance === null){
            Self::$_instance = new PrintfulApiClient('iirs05pd-1hr2-7va8:52yw-kt6uxm4zclvp');
        }
     ...
     {
         if (strlen($key) < 32) {
             throw new Exceptions\PrintfulException('Missing or invalid Printful store key!');
         }
         $this->key = $key;
     }
     /**
     ...
     private function request($method, $path, array $params = [], $data = null)
     {
         ...
         curl_close($curl);
         if ($errorNumber) {
             throw new Exceptions\PrintfulException('CURL: ' . $error, $errorNumber);
         }
         $this->lastResponse = $response = json_decode($this->lastResponseRaw, true);
         if (!isset($response['code'], $response['result'])) {
             $e = new Exceptions\PrintfulException('Invalid API response');
             $e->rawResponse = $this->lastResponseRaw;
             throw $e;
         }
         $status = (int)$response['code'];
         if ($status < 200 || $status >= 300) {
             $e = new Exceptions\PrintfulApiException((string)$response['result'], $status);
             $e->rawResponse = $this->lastResponseRaw;
             throw $e;
         }
